#!/usr/bin/env python3
"""
Generate report figures from the model outputs saved in output_full_t10/predictions.csv.
Creates PNGs similar to tools/generate_report_figures.py but for the full predictions file.

Usage: python3 tools/generate_report_figures_from_predictions.py
"""
import os
import sys
import numpy as np
...
if 'predicted_label' in df.columns and 'actual_label' in df.columns:
    ...
    plt.title('Predictions Confusion matrix (normalized by true label)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.tight_layout()
    fn1 = os.path.join(OUT_DIR, 'predictions_confusion_matrix_normalized.png')
    plt.savefig(fn1, dpi=150)
    plt.close()

    plt.figure(figsize=(6,4))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Oranges')
    plt.title('Predictions Confusion matrix (counts)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.tight_layout()
    fn2 = os.path.join(OUT_DIR, 'predictions_confusion_matrix_counts.png')
    ...
    plt.title('Predictions Actual label distribution')
    plt.tight_layout()
    fn3 = os.path.join(OUT_DIR, 'predictions_label_distribution.png')
...
# Predicted vs Actual scatter and numeric metrics
try:
...
plt.scatter(y, yhat, alpha=0.6, s=8)
...
plt.tight_layout()
fn4 = os.path.join(OUT_DIR, 'predictions_pred_vs_actual.png')
...
plt.legend()
plt.title('Predictions Residuals (prediction - actual)')
plt.tight_layout()
fn5 = os.path.join(OUT_DIR, 'predictions_residuals_hist.png')
plt.savefig(fn5, dpi=150)
plt.close()
print('Saved:', fn5)

# Mean actual vs predicted by time window
agg = None
...
if agg is not None and len(agg)>0:
    ...
    plt.title('Mean Actual vs Predicted by Time Window (predictions.csv)')
    plt.xlabel(group_col)
    plt.legend()
    plt.tight_layout()
    fn6 = os.path.join(OUT_DIR, 'predictions_mean_by_timewindow.png')
